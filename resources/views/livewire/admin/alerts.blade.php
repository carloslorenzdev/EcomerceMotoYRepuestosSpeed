@section('title', 'Alertas de Stock Crítico')
...
<div class="space-y-6">
    ...
    @if (session()->has('toast'))
        <div x-data="{ show: true }" x-show="show" x-init="setTimeout(() => show = false, 4000)" 
            ...
            <span class="font-bold mr-2">Aviso:</span>
    ...
    <div class="rounded-2xl border border-gray-200 bg-white dark:border-neutral-800 dark:bg-neutral-900 p-5 shadow-sm">
        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            ...
            <div class="text-xs font-semibold text-rose-600 dark:text-rose-450 bg-rose-50 dark:bg-rose-950/20 px-4 py-2 rounded-xl border border-rose-100 dark:border-rose-900/50 flex items-center gap-1.5">
                ...
                Solo se muestran productos con stock menor o igual a 5 unidades.
            </div>
                ...
                <tbody class="divide-y divide-gray-150 dark:divide-neutral-800 bg-white dark:bg-neutral-900/50">
                    @forelse ($products as $prod)
                        <tr class="align-middle">
                            ...
                            <!-- Product Name -->
                            <td class="px-4 py-4 font-medium text-gray-800 dark:text-neutral-200">
                                {{ $prod->name }}
                            </td>
                            ...
                            <td class="px-4 py-4 text-center">
                                <span class="inline-flex rounded-full px-2.5 py-0.5 text-[10px] font-bold uppercase tracking-wider
                                    {{ $prod->stock > 0 ? 'bg-amber-100 text-amber-800 dark:bg-amber-950/40 dark:text-amber-400' : 'bg-red-100 text-red-800 dark:bg-red-950/40 dark:text-red-400' }}">
                                    {{ $prod->stock === 0 ? 'Agotado' : "{$prod->stock} unidades" }}
                                </span>
                            </td>
                    ...
                    @empty
                        <tr>
                            <td colspan="5" class="px-4 py-8 text-center text-gray-400">
                                ¡Enhorabuena! No hay alertas de stock crítico.
                            </td>
                        </tr>
                    @endforelse
                </tbody>
